...
To avoid competing systems, the stack is fixed as follows:

- **Authentication: `tymon/jwt-auth` only.** A single JWT (with refresh token) works for both the Next.js web app and the Expo mobile app, matching the `/auth/login` + `/auth/refresh` contract in `docs/03_API_SPECIFICATION.md`. Laravel Sanctum is **not** used (it lacks the refresh-token flow the clients rely on and would duplicate auth responsibilities).
- **Authorization: custom RBAC, not `spatie/laravel-permission`.** The `roles`, `permissions`, `role_permissions`, and `user_roles` tables in `database/schema.sql` (with the `HasRoles` trait and dynamic `Gate` definitions in `backend-specs/rbac_permissions.md`) are the source of truth. The Spatie package is intentionally excluded because its table shape differs from our org-scoped, custom permission model.
- **`users.role` enum** remains as a denormalized "primary role" for fast, common-case checks and for seeding; granular access is always resolved through the RBAC tables.

Two authorization middleware aliases exist and may both be used:
- `ensure.role:admin|dispatcher` — coarse role gate (cheap, for whole route groups).
- `permission:routes.view` — granular permission gate (for specific actions).
...
Route::prefix('v1')->group(function () {
    ...
    Route::middleware('auth:api')->group(function () {
        ...
        Route::delete('/auth/devices/{token}', [AuthController::class, 'unregisterDevice']);
        ...
        Route::post('runs/{run}/optimize', [RunController::class, 'optimize']);
        Route::get('runs/{run}/optimization', [RunController::class, 'optimization']);
        Route::post('runs/{run}/optimization/apply', [RunController::class, 'applyOptimization']);
        ...
        Route::prefix('parent')->group(function () {
            Route::get('/profile', [ParentPortalController::class, 'profile']);
            Route::put('/profile', [ParentPortalController::class, 'updateProfile']);
            Route::get('/children', [ParentPortalController::class, 'children']);
            Route::post('/children/link', [ParentPortalController::class, 'linkChild']);
            Route::delete('/children/{student}', [ParentPortalController::class, 'unlinkChild']);
            Route::get('/children/{student}/routes', [ParentPortalController::class, 'childRoutes']);
            Route::get('/children/{student}/tracking', [ParentPortalController::class, 'tracking']);
            Route::get('/notifications', [ParentPortalController::class, 'notifications']);
            Route::put('/notification-prefs', [ParentPortalController::class, 'updatePrefs']);
            Route::get('/history', [ParentPortalController::class, 'history']);
            Route::post('/on-demand', [ParentPortalController::class, 'submitOnDemand']);
        });

        // Billing
        Route::prefix('billing')->group(function () {
            Route::apiResource('rates', BillingController::class)->only(['index', 'store', 'update', 'destroy']);
            Route::get('items', [BillingController::class, 'items']);
            Route::post('items/generate', [BillingController::class, 'generateItems']);
            Route::apiResource('invoices', BillingController::class)->only(['index', 'store', 'show']);
            Route::put('invoices/{invoice}/send', [BillingController::class, 'sendInvoice']);
            Route::put('invoices/{invoice}/pay', [BillingController::class, 'recordPayment']);
