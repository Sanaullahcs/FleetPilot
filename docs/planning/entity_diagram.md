...
erDiagram
    ...
    USERS ||--o| PARENT_ACCOUNTS : extends
    USERS ||--o{ APP_DEVICES : registers
    PARENT_ACCOUNTS ||--o{ PARENT_STUDENTS : links
    STUDENTS ||--o{ PARENT_STUDENTS : linked_to

    RUNS ||--o{ ROUTE_OPTIMIZATIONS : optimized_by

    RUN_ASSIGNMENTS ||--o{ BILLING_ITEMS : bills
    BILLING_RATES ||--o{ BILLING_ITEMS : priced_by
    DRIVERS ||--o{ BILLING_ITEMS : earns
    INVOICES ||--o{ BILLING_ITEMS : groups
    DRIVERS ||--o{ INVOICES : invoiced_to
...
users
    ├── parent_accounts [1:1]
    │   └── parent_students [1:N] → students [N:1]
    └── app_devices [1:N]   (push tokens)
```

#### 8. Billing & Optimization
```
runs
    └── route_optimizations [1:N]
run_assignments
    └── billing_items [1:N] → billing_rates [N:1]
invoices
    └── billing_items [1:N]   (grouped by driver + period)
```
...
organizations
    ├── roles [1:N]
    └── permissions [1:N]
roles ──< role_permissions >── permissions   (M:N)
users ──< user_roles >── roles               (M:N, with assigned_by audit)
```
...
*Generated for FleetPilot (K-12 Transportation MVP) | 2026-06-05*
